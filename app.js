...
function renderCertificateOverlay(certificateData, docId) {
    ...
    <style>
        @media print {
            ...
            #certificateOverlay { position: static !important; background: white !important; padding: 0 !important; }
            .cert-container { box-shadow: none !important; border-radius: 0 !important; max-width: 100% !important; }
    ...
    <div id="certificateOverlay" class="fixed inset-0 bg-black/95 flex items-center justify-center z-[10000] p-3 sm:p-6 overflow-hidden">
        <div class="cert-container bg-white w-full max-w-lg sm:max-w-3xl md:max-w-4xl rounded-3xl shadow-2xl overflow-hidden text-slate-900" style="max-height: 94vh;">
            
            <!-- Nagłówek -->
            <div class="pt-6 sm:pt-10 pb-5 sm:pb-8 text-center border-b border-slate-200">
                <img src="logo.png" alt="MyHeredo" class="h-14 sm:h-20 mx-auto mb-4 sm:mb-6">
                <h1 class="text-2xl sm:text-4xl font-bold flex items-center justify-center gap-2 sm:gap-3">
                    <span class="text-xl sm:text-3xl">🪶</span> CERTYFIKAT SUKCESJI
                </h1>
                <p class="text-base sm:text-xl text-amber-600 font-medium mt-1">Cyfrowa Sukcesja</p>
                <p class="text-slate-600 text-sm sm:text-base">MyHeredo • Bezpieczny Sejf Spadkowy</p>
            </div>
            
            <!-- Treść -->
            <div class="p-5 sm:p-10 space-y-6 sm:space-y-9 overflow-auto" style="max-height: calc(94vh - 240px);">
                <div class="grid grid-cols-2 gap-4 sm:gap-8 text-sm">
                    <div>
                        ...
                        <p class="font-mono font-bold text-base sm:text-2xl break-all">${docId}</p>
                    </div>
                    <div class="text-right">
                        <p class="text-xs uppercase tracking-widest text-slate-500">Data wystawienia</p>
                        <p class="text-base sm:text-xl">${formattedDate}</p>
                    </div>
                </div>

                <div>
                    <p class="text-xs uppercase tracking-widest text-slate-500">Właściciel sejfu</p>
                    <p class="text-lg sm:text-2xl font-semibold break-all">${certificateData.ownerEmail}</p>
                </div>

                <div>
                    <p class="text-xs uppercase tracking-widest text-slate-500">Dead Man’s Switch</p>
                    <p class="text-lg sm:text-2xl font-semibold">${certificateData.dmsDays || 45} dni bezczynności</p>
                </div>

                <!-- Skrytki -->
                <div>
                    <p class="text-xs uppercase tracking-widest text-slate-500 mb-4 sm:mb-5">ZABEZPIECZONE SKRYTKI</p>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                        ${vaults.map(v => {
                            ...
                            return `
                                <div class="flex items-center gap-3 sm:gap-4 bg-slate-50 border border-slate-200 p-4 sm:p-5 rounded-2xl">
                                    <span class="text-3xl sm:text-4xl flex-shrink-0">${icon}</span>
                                    <div class="min-w-0">
                                        <p class="font-medium text-sm sm:text-base leading-tight">${v.category}</p>
                                        <p class="text-emerald-600 text-xs sm:text-sm">Zaszyfrowane</p>
                                    </div>
                                </div>
                            `;
                        }).join('')}
                ...
                <!-- Spadkobiercy -->
                <div>
                    <p class="text-xs uppercase tracking-widest text-slate-500 mb-4 sm:mb-5">SPADKOBIERCY (${certificateData.heirs ? certificateData.heirs.length : 0})</p>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                        ${(certificateData.heirs || []).map(h => `
                            <div class="bg-slate-50 border border-slate-200 p-5 sm:p-6 rounded-2xl">
                                <p class="font-semibold text-sm sm:text-base">${h.name}</p>
                                <p class="text-slate-600 text-xs sm:text-sm break-all">${h.email}</p>
                                <p class="text-emerald-600 text-xs sm:text-sm mt-3">● Pełny dostęp</p>
                            </div>
                        `).join('')}
                    </div>
                </div>
            ...
            <!-- Przyciski -->
            <div class="border-t p-5 sm:p-8 flex flex-col gap-3 print:hidden">
                <button onclick="printCertificate()" class="w-full py-4 sm:py-6 bg-slate-900 text-white font-semibold rounded-2xl text-base sm:text-lg hover:bg-black transition">🖨️ Drukuj / Zapisz jako PDF</button>
                <button onclick="decryptCertificate('${docId}')" class="w-full py-4 sm:py-6 bg-emerald-600 text-white font-semibold rounded-2xl text-base sm:text-lg hover:bg-emerald-700 transition">🔓 Odszyfruj Skrytki</button>
                <button onclick="closeCertificate()" class="w-full py-4 sm:py-6 border border-slate-300 font-semibold rounded-2xl text-base sm:text-lg hover:bg-slate-100 transition">Zamknij</button>
            </div>
        </div>
    </div>`;
